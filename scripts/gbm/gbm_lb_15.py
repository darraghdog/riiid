...
def add_user_feats(df, pdicts, kdicts, update = True):
    ...
    tstamp = np.zeros((len(df), 5), dtype=np.int32)
    tstavg = np.zeros(len(df), dtype=np.float32)
    lectct = np.zeros((len(df), 7), dtype=np.int32)
    ...
    for cnt,row in enumerate(tqdm(df[itercols].values, total = df.shape[0]) ):
        ...
        if ctype==1:
            ...
            if pdicts['lect_time_mat'][ukey, 5] == 1: #if pdicts['cum_ques_since_lecture'][u] == 1:
                time_since = int(round(tstmp - pdicts['lect_time_mat'][ukey, 2])) #    time_since = int(round(tstmp - pdicts['prev_lecture_timestamp'][u]))/1000
    ...
    for t1, (matcu, matascu) in enumerate(zip([cu, expcu, cidcu, cb], 
        ...
        df[f'avgcorrect___feat{t1}'] =  (matascu / (matcu + 0.001)).astype(np.float32)
        #gc.collect()
    df['cid_answered_correctly'] = acsu
    df[[f'lag_content_time{i}' for i in [0,1,2,5,10]]] = tstamp.astype(np.int32)
...
pdicts = {
          ...
          'lag_time': np.zeros((int(kdicts['userCtr'] *1.2) , 10), dtype= np.int32),
          'lag_time_cum': np.zeros((int(kdicts['userCtr'] *1.2) , 1), dtype= np.uint64),
          'lect_time_mat': np.zeros((int(kdicts['userCtr'] *1.2) , 8), dtype= np.int32),
