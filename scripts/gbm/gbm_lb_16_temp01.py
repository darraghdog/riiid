...
first_question = train.drop_duplicates(subset='user_id', keep='first').set_index('user_id').content_id
...
def get_oof(clf, x_train, y, x_test, folds):
    NFOLDS = len(folds.unique())
    oof_train = np.zeros((x_train.shape[0],))
    oof_test = np.zeros((x_test.shape[0],))
    oof_test_skf = np.empty((NFOLDS, x_test.shape[0]))

    for i, f in enumerate(range(NFOLDS)):
        train_index, test_index = np.where(f!=folds), np.where(f==folds)
        print('\nFold {}'.format(i))
        ...
        oof_test_skf[f, :] = clf.predict(x_test)
        gc.collect()
        del x_tr, x_te, train_index, test_index, y_tr
        gc.collect()
    oof_test[:] = oof_test_skf.mean(axis=0)
    return oof_train.reshape(-1, 1), oof_test.reshape(-1, 1)

SEED    = 200
ridge_params = { 'alpha':0.000001, 'fit_intercept':True, 'normalize':True, 'copy_X':True,
                'max_iter':None, 'tol':0.001, 'solver':'auto', 'random_state':SEED }
ridge = SklearnWrapper(clf=Ridge, seed = SEED, params = ridge_params)
#ridge_oof_train, ridge_oof_test = get_oof(ridge, ready_df[:ntrain], y, ready_df[ntrain:])
usefeats = list(set(FEATS) - set(CATEGORICALS) - set(['prior_question_elapsed_time']))
folds = train.user_id % 5
ridge_oof_train, ridge_oof_test = get_oof(ridge \
                                          , train[set(usefeats)].values \
                                          , train[TARGET] \
                                          , valid[set(usefeats)].values \
                                          , folds)
train['ridge_oof'] = ridge_oof_train
valid ['ridge_oof'] = ridge_oof_test
