...
def add_user_feats(df, pdicts, update = True):
    ...
    for cnt,row in enumerate(tqdm(df[itercols].values, total = df.shape[0]) ):
        ...
        if ctype==1:
            lectcid, lpart, ltype_of, ltag = cid, ldict['part'][cid], ldict['type_of'][cid], ldict['tag'][cid]
            ...
            try:
                pdicts['lecture_tags'][u].add(ltag)
            except:
                pdicts['lecture_tags'][u] = set([ltag])
            continue
        else:
        ...
        if update:
            ...
            pdicts[f'prior_question_elapsed_time_lag0'][u] = eltim
...
'''
cdf = train.query('content_type_id == 0')[['user_id', 'task_container_id', 'content_id', 'answered_correctly']].drop_duplicates()
ccf =  cdf.head(1000000).groupby(['user_id', 'task_container_id'])['answered_correctly'].agg('mean', 'count')
cdf =  cdf.head(1000000).groupby(['user_id', 'task_container_id'])['content_id'].apply(list)

cdf = pd.merge(ccf, cdf, left_index = True, right_index = True)

cdf = cdf.reset_index().drop('task_container_id', 1)
cdf.content_id = cdf.content_id.apply(lambda x: '|'.join(map(str,x)) )

cdf[cdf.str.contains('2065')].groupby('content_id')['answered_correctly'].agg(['mean', 'count'])

cdf.content_id.value_counts().shape
cdf[cdf.str.contains('2065')].value_counts()
'''
...
pdicts = {'answered_correctly_sum_u_dict' : defaultdict(int),
          ...
          'content_id_prev_qa_ratio': defaultdict(int),
...
for i in range(5): 
    pdicts[f'prior_question_elapsed_time_lag{i}'] =   defaultdict(int)
...
train.part.value_counts(normalize = True).sort_index().round(3)
first_part = pd.concat([train[['user_id', 'part']], valid[['user_id', 'part']]], 0) \
           .drop_duplicates(subset='user_id').set_index('user_id').part
           
train['first_part'] = first_part.loc[train.user_id].values
valid['first_part'] = first_part.loc[valid.user_id].values

train['first_part'] .value_counts()
...
FEATS = ['answered_correctly_avg_c', 'attempts_avg_c', 'answered_correctly_first_avg_c', 'cid_answered_correctly', \
         ...
         'answered_correctly_ct_c', 'answered_correctly_last_avg_c', 'lag_content_avgtime', 'timestamp']
...
lgb_train = lgb.Dataset(train[FEATS], y_tr, categorical_feature = CATEGORICALS, free_raw_data = False)
lgb_valid = lgb.Dataset(valid[FEATS], y_va, categorical_feature = CATEGORICALS, free_raw_data = False)
_=gc.collect()
...
[100]	training's binary_logloss: 0.521979	valid_1's binary_logloss: 0.53643
[200]	training's binary_logloss: 0.517963	valid_1's binary_logloss: 0.533581
[300]	training's binary_logloss: 0.515829	valid_1's binary_logloss: 0.532319
[400]	training's binary_logloss: 0.514256	valid_1's binary_logloss: 0.531474
[500]	training's binary_logloss: 0.51293	valid_1's binary_logloss: 0.530872
[600]	training's binary_logloss: 0.5117	valid_1's binary_logloss: 0.530402
[700]	training's binary_logloss: 0.510736	valid_1's binary_logloss: 0.529998
[800]	training's binary_logloss: 0.509962	valid_1's binary_logloss: 0.52979
[900]	training's binary_logloss: 0.509211	valid_1's binary_logloss: 0.529526
[1000]	training's binary_logloss: 0.50839	valid_1's binary_logloss: 0.52929
[1100]	training's binary_logloss: 0.507678	valid_1's binary_logloss: 0.529139
[1200]	training's binary_logloss: 0.506963	valid_1's binary_logloss: 0.529
[1300]	training's binary_logloss: 0.506482	valid_1's binary_logloss: 0.528926
[1400]	training's binary_logloss: 0.505904	valid_1's binary_logloss: 0.528812
[1500]	training's binary_logloss: 0.505357	valid_1's binary_logloss: 0.528718
[1600]	training's binary_logloss: 0.504835	valid_1's binary_logloss: 0.52864
[1700]	training's binary_logloss: 0.504186	valid_1's binary_logloss: 0.528502
Early stopping, best iteration is:
[1697]	training's binary_logloss: 0.504205	valid_1's binary_logloss: 0.5285
auc: 0.7889330320023249
'''
