...
class XLMEduNet(nn.Module):
    def __init__(self, modcols, contcols, padvals, extracols, 
                 ...
                 hidden = args.hidden, n_repeats = 4):
        ...
        self.emb_bundle_id = nn.Embedding(13526, 64)
        self.emb_part = nn.Embedding(9, 32)
        self.emb_tag= nn.Embedding(190, 32)
        self.emb_lpart = nn.Embedding(9, 8)
        ...
        IN_UNITSQA = self.emb_lag_time.embedding_dim + self.emb_elapsed_time.embedding_dim + \
                ...
                self.pos_embedding.embedding_dim + len(self.contcols) * self.n_repeats
        LSTM_UNITS = hidden
    ...
    def forward(self, x, m = None):
        ...
        contmat = torch.repeat_interleave(contmat, self.n_repeats, -1)
