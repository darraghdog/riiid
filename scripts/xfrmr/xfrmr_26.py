...
class LearnNet4(nn.Module):
    def __init__(self, modcols, contcols, padvals, extracols, 
        ...
        self.seqnet2 = nn.LSTM(IN_UNITSQA + LSTM_UNITS, 
                               LSTM_UNITS, bidirectional=False, batch_first=True)
        ...
        self.linear1a = nn.Linear(LSTM_UNITS*2, LSTM_UNITS*2)
        self.linear1b = nn.Linear(LSTM_UNITS*2, LSTM_UNITS*2)
    ...
    def forward(self, x, m = None):
        ...
        hiddenq_attend, _ = self.atten2(hiddenq, m)
        hiddenqa_attend, _ = self.atten2(hiddenqa, m)
        
        # Take last hidden unit
        h_conc = torch.cat((hiddenq_attend , hiddenqa_attend), 1)
        h_conc = self.dropout( self.bn1(h_conc) )
        h_conc_linear1  = F.relu(self.linear1a(h_conc))
        h_conc_linear2  = F.relu(self.linear1b(h_conc))
