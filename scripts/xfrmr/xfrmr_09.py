...
pdicts['keepcols'], pdicts['NORMCOLS']  = keepcols, NORMCOLS
...
class SAKTDataset(Dataset):
    def __init__(self, data, basedf, cols, padvals, extracols, 
        ...
        self.final_container_dummy = 13523*4 + 1
        self.task_container_id = self.data.task_container_id.values
        self.content_user_answer_idx = self.cols.index('content_user_answer')
    ...
    def __getitem__(self, idx, row = None):
        ...
        if self.submit:
            ...
            umat[:, self.content_user_answer_idx][dummy_answer] = self.final_container_dummy
...
trndataset = self = SAKTDataset(train, None, pdicts['MODCOLS'], pdicts['PADVALS'], pdicts['EXTRACOLS'], maxseq = args.maxseq)
