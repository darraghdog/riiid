...
class LearnNet31(nn.Module):
    def __init__(self, modcols, contcols, padvals, extracols, 
        ...
        self.emb_cont_correct_answer = nn.Embedding(13526 * 2, 16)
        ...
        IN_UNITSQ = self.diffsize * 2 + len(self.contcols)
        ...
        IN_UNITSQA = ( self.emb_lag_time.embedding_dim + self.emb_elapsed_time.embedding_dim + \
                self.emb_cont_user_answer.embedding_dim) + self.emb_cont_correct_answer.embedding_dim #+ #len(self.contcols)
        LSTM_UNITS = hidden 
        self.diffsize = self.emb_content_id.embedding_dim + self.emb_part.embedding_dim + \
                        self.emb_bundle_id.embedding_dim + self.emb_tag.embedding_dim * 7 
    ...
    def forward(self, x, m = None):
        ...
        embcatq = torch.cat([
            ...
            , 2)
        embcatqdiff = embcatq - embcatq[:,-1,:].unsqueeze(1)
        ...
        xinpq = torch.cat([embcatq, embcatqdiff, contmat], 2)
        hiddenq, _ = self.seqnet1(xinpq)
        xinpqa = torch.cat([embcatqa, hiddenq], 2)
