...
test = valid.groupby(['user_id']).apply(lambda x : x.tail(random.randint(0, 50)))
valid = valid[~valid.row_id.isin(test.row_id)]
if args.dumpdata:
    dumpobj(f'data/{DIR}/test_{VERSION}_pre.pk', test)
...
test = pd.merge(test, qdf[keepcols], left_on = 'content_id', right_on = 'question_id', how = 'left')
...
test[formatcols] = test[formatcols].fillna(0).astype(np.int16)
...
test['prior_question_had_explanation'] = test['prior_question_had_explanation'].astype(np.float32).fillna(2).astype(np.int8)
...
test['prior_question_elapsed_time'] = test['prior_question_elapsed_time'].fillna(0).astype(np.int32)
...
test = pd.merge(test, content_df, on=['content_id'], how="left")
...
test['task_container_cts'] = test[taskcols][::-1].groupby(taskcols).cumcount()[::-1]
...
u_int_cols = ['answered_correctly_sum_u_dict', 'count_u_dict', \
              'content_id_lag',  'pexp_count_u_dict'] #'track_b', 'answered_correctly_sum_b_dict',  'count_b_dict', 
...
pdicts_test = copy.deepcopy(pdicts)
test = add_user_feats(test, pdicts_test)
...
test['content_user_answer']  = test['user_answer'] + 4 * test['content_id'].astype(np.int32)
...
test['answered_correctly_ct_log'] = np.log1p(test['answered_correctly_ct_c'].fillna(0).astype(np.float32)) - 7.5
...
train[pdicts['NORMCOLS']] = np.log1p(train[pdicts['NORMCOLS']].fillna(0).astype(np.float32)) - meanvals
valid[pdicts['NORMCOLS']] = np.log1p(valid[pdicts['NORMCOLS']].fillna(0).astype(np.float32)) - meanvals
test[pdicts['NORMCOLS']] = np.log1p(test[pdicts['NORMCOLS']].fillna(0).astype(np.float32)) - meanvals
...
tstidx = test.reset_index().groupby(['user_id'])['index'].apply(list).to_dict()
...
if args.dumpdata:
    ...
    for k, v in pdicts_test.items():
        dumpobj(f'data/{DIR}/pdicts_test____{VERSION}_{k}.pk', v)
    ...
    for cid, d in enumerate(tqdm(pdicts['uqidx'])):
        for u, i in d.items():
            ...
            fo.write(s)
    fo.close()	
    fo = open(f'data/{DIR}/pdicts_test____{VERSION}_uqidx.csv','w')
    for cid, d in enumerate(tqdm(pdicts_test['uqidx'])):
        for u, i in d.items():
            s = f'{u} {cid} {i}\n'
            fo.write(s)
    fo.close()
    ...
    #dumpobj(f'data/{DIR}/test_{VERSION}.pk', valid)
...
tstdataset = SAKTDataset(test , pd.concat([train, valid]), **daargs)
...
if args.dumpdata:
    ...
    dumpobj(f'data/{DIR}/tstdataset_{VERSION}_tail.pk', tstdataset)
    dumpobj(f'data/{DIR}/test_{VERSION}_post.pk', test)
    del df, alldataset 
    gc.collect()
...
for epoch in range(args.epochs):
    ...
    pbartst = tqdm(enumerate(tstloader), 
                total = len(tstdataset)//loaderargs['batch_size'], 
                desc=f"Test epoch {epoch}", ncols=0)
    y_predls = []
    y_act = test['answered_correctly'].values
    for step, batch in pbartst:
        x, m, y = batch
        x = x.to(device, dtype=torch.float)
        m = m.to(device, dtype=torch.long)
        with torch.no_grad():
