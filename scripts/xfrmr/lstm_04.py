...
arg('--user_max', type=int, default=512)
...
class SAKTDataset(Dataset):
    def __init__(self, data, basedf, cols, padvals, extracols, carryfwdcols, 
        ...
        quidx = self.data.query('base==0').reset_index()[['user_id', 'index']]
        quidx['userct'] = quidx.groupby('user_id').transform('count')
        quidx['userwt'] = quidx['userct'].clip(0, args.user_max) /  quidx['userct']         
        sampwts = torch.tensor(quidx.userwt.values)
        self.quidx = quidx.reset_index()[['user_id', 'index']].values
        self.sampwts = torch.tensor(quidx.userwt.values)
