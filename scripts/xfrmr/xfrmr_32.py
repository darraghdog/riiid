...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, idx, row = None):
        ...
        if self.has_target:
            ...
            qaidx = umat[-1,[self.cols.index('content_id'), self.cols.index('user_answer')]].astype(np.int32)
            try:
                targetRatio = qaRatio[tuple(qaidx)]
            except:
                targetRatio = 0.5
        ...
        return umat, umask, target, targetRatio
...
class LearnNet32(nn.Module):
    def __init__(self, modcols, contcols, padvals, extracols, 
        ...
        self.emb_cont_correct_answer = nn.Embedding(13526 * 2, 8)
        ...
        self.linear1qa = nn.Linear(LSTM_UNITS * 2 + len(self.contcols), LSTM_UNITS//2)
        ...
        self.bn1qa = nn.BatchNorm1d(num_features=LSTM_UNITS * 2 + len(self.contcols))
        self.bn2 = nn.BatchNorm1d(num_features=LSTM_UNITS//2)
        self.bn2qa = nn.BatchNorm1d(num_features=LSTM_UNITS//2)
        
        self.linear_out = nn.Linear(LSTM_UNITS//2, 1)
        self.linear_outqa = nn.Linear(LSTM_UNITS//2, 1)
        
    def forward(self, x, m = None, qa = False):
        ...
        embcatqa = torch.cat([
            ...
            #self.emb_bundle_id(  x[:,:, self.modcols.index('bundle_id')].long()  ),
            self.emb_cont_user_answer(  x[:,:, self.modcols.index('content_user_answer')].long()  ),
            self.emb_cont_correct_answer(  correctidx ),
            self.emb_lag_time(   x[:,:, self.modcols.index('lag_time_cat')].long()   ), 
            self.emb_elapsed_time(  x[:,:, self.modcols.index('elapsed_time_cat')].long()  )
            ] #+ [self.emb_tag(x[:,:, ii.item()].long()) for ii in torch.where(self.tag_idx)[0]]
            , 2)
        ...
        hiddenall = torch.cat([hiddenqa[:,-1,:], hiddenq[:,-1,:], contmat[:, -1]], 1)
        
        if qa:
            hiddenqa = self.dropout( self.bn1qa( hiddenall) )
            hiddenqa  = F.relu(self.linear1qa(hiddenqa))
            hiddenqa = self.dropout(self.bn2qa(hiddenqa))
            outqa = self.linear_outqa(hiddenqa).flatten()
        
        hidden = self.dropout( self.bn1( hiddenall) )
...
for epoch in range(args.epochs):
    ...
    for step, batch in pbartrn:
        ...
        yqa = yqa.to(device, dtype=torch.float)
