...
def add_user_feats(df, pdicts, update = True):
    ...
    outmat = np.concatenate((countmat, avgcorrectmat, acsumat, qamat, lect, lectcat, partcu, avgcorrectpartmat), 1)
...
# + [f'part{i}_ct_avg_correct' for i in range(8)] 
...
pdicts['EXTRACOLS'] = ['lag_time_cat',  'elapsed_time_cat', 'lag_5', 'lag_10']
...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, idx, row = None):
        ...
        # Lag time
        umatlag = np.zeros((args.maxseq, 2), dtype = np.float32)
        umatlag[5:, 0] = umat[:, self.timecols[0]][:-5]
        umatlag[10:,1] = umat[:, self.timecols[0]][:-10]
        # preprocess continuous time - try log scale and roughly center it
        umatlag = np.log10( 1.+ umatlag / 60  ) 
        ...
        umat = np.concatenate((umat, timeemb, umatlag), 1)
...
class LearnNet2(nn.Module):
    def __init__(self, modcols, contcols, padvals, extracols, 
        ...
        self.contcols = contcols + ['lag_5', 'lag_10']
...
pdicts['daargs'] = daargs = {'cols':pdicts['MODCOLS'], 
          ...
          'extracols':pdicts['EXTRACOLS'], 
          'maxseq': args.maxseq}
trndataset = self = SAKTDataset(train, None, **daargs)
...
if args.dumpdata:
    ...
    dumpobj(f'data/{DIR}/alldataset_{VERSION}_full_backup.pk', alldataset)
    alldataset.quidx = None
    alldataset.task_container_id = None
    fo = open(f'data/{DIR}/alldataset__manual_{VERSION}_uidx.csv','w')
    for t, (u, posls) in enumerate(tqdm(alldataset.uidx.items())):
        for pos in posls:
            s = f'{u} {pos}\n'
            fo.write(s)
    fo.close()
    alldataset.uidx = None
